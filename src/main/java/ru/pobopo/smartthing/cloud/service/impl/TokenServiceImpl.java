...
public class TokenServiceImpl implements TokenService {
    ...
    public String generateToken(UserEntity user, GatewayEntity gatewayEntity)
        ...
        tokenRepository.save(tokenInfoEntity);
        String token = generateToken(tokenInfoEntity);

        log.warn("Sending gateway login event");
        applicationEventPublisher.publishEvent(new GatewayLoginEvent(this, gatewayEntity));
    ...
    public void deactivateToken(String tokenId) throws AccessDeniedException, AuthenticationException {
        ...
        TokenInfoEntity oldToken = tokenInfoEntity.get();
        deactivateToken(oldToken);

        if (TokenType.GATEWAY.getName().equals(oldToken.getType())) {
            log.warn("Sending gateway logout event");
            applicationEventPublisher.publishEvent(new GatewayLogoutEvent(this, oldToken.getGateway()));
        }
    }
    ...
    @Override
    public boolean isAuthorized(GatewayEntity gateway) {
        if (gateway == null) {
            return false;
        }

        return tokenRepository.findByActiveAndGateway(true, gateway) != null;
    }
